...
def limpiar_y_parsear_guion_bmad(ruta_archivo):
    ...
    for linea in lineas:
        ...
        # Omitir títulos de Markdown (#, ##, ###) y separadores (---)
        if linea_str.startswith("#") or linea_str.startswith("-"):
            continue

        # Omitir directrices visuales o técnicas encerradas entre [] o ()
        if linea_str.startswith("[") or linea_str.endswith("]") or linea_str.startswith("("):
            continue

        # Detectar la nota de exención de responsabilidad y programar silencio
        if (
            "exención de responsabilidad" in linea_low
            or "nota de exención" in linea_low
            or "puramente divulgativo" in linea_low
        ):
        ...
        # Limpiar símbolos de formato y etiquetas de diálogo
        linea_saneada = re.sub(r"[\*\#\_\-\`\[\]\(\)\>\<\🖳]", "", linea_str)
        for palabra in palabras_tecnicas:
            linea_saneada = re.sub(
                r"(?i)^\s*" + re.escape(palabra) + r"\s*\:?\s*",
                "",
                linea_saneada,
            )
    ...
    try:
        for idx, elemento in enumerate(bloques_guion, 1):
            ...
            if elemento == "[INJECT_SILENCE_5_SECONDS]":
                print(
                    f"🤫 [Bloque {idx}/{len(bloques_guion)}] "
                    "Inyectando 5 segundos de silencio digital perfecto..."
                )
                audio_unificado += AudioSegment.silent(duration=5000)
                continue

            print(
                f"🎙️ [Bloque {idx}/{len(bloques_guion)}] "
                "Creando archivo temporal e invocando OpenAI TTS..."
            )
            ...
            audio_unificado += AudioSegment.from_mp3(temp_chunk_path)
            time.sleep(0.4)
        ...
        print("\n" + "=" * 50)
        print("🎉 ¡FASE 3 FINALIZADA CON ÉXITO ROTUNDO!")
